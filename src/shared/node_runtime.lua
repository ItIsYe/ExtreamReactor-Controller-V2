...
-- Verteilte Architektur: wireless = Kommunikation, wired = lokale Hardware,
-- Nodes laufen autonom weiter, Master ist optional/ersetzbar
--========================================================
...
function Runtime.create(opts)
  ...
  function self:run_event_loop()
    event_loop()
  end

  function self:start()
    local function run_dispatcher()
      if dispatcher.is_running and dispatcher:is_running() then
        local wait = dispatcher.receive_wait or 0.2
        while running do sleep(wait) end
      else
        dispatcher:start()
      end
    end

    parallel.waitForAny(run_dispatcher, event_loop)
  end
