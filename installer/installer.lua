...
local function writeStartup(targetPath)
  ...
  if targetPath:match("^/rom/") then
    error("Startup target cannot be in ROM: " .. targetPath)
  end
  ...
  if fs.exists(STARTUP_PATH) then
    if fs.isDir and fs.isDir(STARTUP_PATH) then
      error("Cannot replace directory at startup path: " .. STARTUP_PATH)
    end
  ...
  if not handle then
    error("Unable to open " .. STARTUP_PATH .. " for writing")
  end

  local ok, write_err = pcall(handle.write, handle, startup_content(targetPath))
  handle.close()
  if not ok then
    error(write_err)
  end
