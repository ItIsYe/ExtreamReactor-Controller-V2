  ...
  for role, src in pairs(ROLE_SOURCE_FILES) do
    ...
    for _, file in ipairs(manifest.files) do
      if file.src == src then
        ...
        if expected_dst and file.dst ~= expected_dst then
...
local function write_startup(role_name, target)
  ...
  local normalized_role = string.upper(role_name)
  ...
  local contents = table.concat({
    "-- Autogenerated by XReactor installer",
    string.format("local STARTUP_ROLE = %q", normalized_role),
    string.format("local STARTUP_TARGET = %q", target),
    "shell.run(STARTUP_TARGET)",
    "",
  }, "\n")
...
local function verify_startup_file(role_name, target)
  ...
  local shell_runs = {}
  for run_args in content:gmatch("shell%.run%s*%(([^)]*)%)") do
    table.insert(shell_runs, run_args)
  end

  if #shell_runs ~= 1 then
    return false, "startup.lua must contain exactly one shell.run call"
  end

  local run_target = shell_runs[1]:match("[\"'](.-)[\"']")
  ...
  local stored_role = content:match("STARTUP_ROLE%s*=%s*[\"'](.-)[\"']")
  if stored_role then
    if string.upper(stored_role) ~= normalized_role then
      return false, "startup.lua role metadata mismatch"
    end
  else
    return false, "startup.lua is missing role metadata"
  end
